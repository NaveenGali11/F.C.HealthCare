...
if(isset($_POST["email"]) && isset($_POST["password"])){
	...
	$run_query = mysqli_query($con,$sql) or mysqli_error($con);
	...
	if($count == 1){
			...
			if (isset($_COOKIE["product_list"])) {
				...
				for ($i=0; $i < count($product_list); $i++) {
					...
					$result  = mysqli_query($con,$verify_cart) or die(mysqli_error($con));
			...
			$BackToMyPage = $_SERVER['HTTP_REFERER'];
				if(!isset($BackToMyPage)) {
					...
					header('Location: doctors_home.php'); // default page
				}
